...
const postProduct = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ msg: "producte added successfully", details: req.body });
  } catch (err) {
...
const getProduct = async (req, res) => {
  try {
    ...
    const products = await ProductModel.find({ ...query, ...searchQuery }).sort(
      sortQuery
    );
    res
      .status(200)
      .json({ msg: "products successfully fetched", data: products });
  } catch (err) {
    res.status(400).json({ err: err.message });
  }
};
...
const updateProduct = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ msg: "product updated successfully", updatedDetails: req.body });
  } catch (err) {
    res.status(400).json({ err: err.message });
  }
};
const deleteProduct = async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ msg: "product deleted successfully", deletedDetails: req.body });
  } catch (err) {
    res.status(400).json({ err: err.message });
  }
};
